...
Stop if `GITHUB_TOKEN` is missing or the response headers do not include `write:packages`. Do this before creating the kaniko Job so GHCR auth failures are caught in preflight instead of after a long build.
