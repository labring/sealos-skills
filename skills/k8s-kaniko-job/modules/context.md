...
```bash
SHORT_REF="$(node -e 'const r=require(process.argv[1]); process.stdout.write((r.source.ref || "unknown").slice(0, 12))' "$WORK_DIR/.sealos/build/build-request.json")"
...
node "$SKILL_DIR/scripts/prepare-context.mjs" \
  ...
  --devbox "$DEVBOX_NAME" \
  --build-id "$BUILD_ID" \
  --out "$WORK_DIR/.sealos/build/kaniko-context.json"
```
...
$WORK_DIR/.sealos/build/kaniko-context.json
```
...
```bash
tar -tzf "$(node -e 'const m=require(process.argv[1]); process.stdout.write(m.context.tar_path)' "$WORK_DIR/.sealos/build/kaniko-context.json")" | head -100
```

Confirm the Dockerfile path in `.sealos/build/kaniko-context.json.kaniko.dockerfile` exists inside the tar root.
