...
function resolvePodIp(namespace, podName) {
  if (process.env.POD_IP) {
    return { value: process.env.POD_IP, source: 'env' }
  }

  if (namespace && podName) {
    const podIp = runKubectl([
      'get',
      'pod',
      podName,
      '-n',
    ...
    if (podIp) {
      return { value: podIp, source: 'pod-status' }
    }
  }
...
function main() {
  try {
    ...
    const podIp = resolvePodIp(namespace.value, podName)
