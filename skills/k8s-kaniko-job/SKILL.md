...
This skill is the sandbox build executor inside the full `sealos-deploy` flow:

```text
sealos-deploy
  -> .sealos/build/build-request.json
  -> k8s-kaniko-job
  -> .sealos/build/kaniko-context.json
...
- read `.sealos/build/build-request.json`
- accept only `mode=build-required`; the main pipeline skips this executor for reusable images
...
- write `.sealos/build/build-result.json`
...
2. `modules/build-request.md` — read and validate `.sealos/build/build-request.json`
3. `modules/registry-auth.md` — prepare GHCR and S3 auth Secrets
4. `modules/context.md` — generate `context.tar.gz` and `.sealos/build/kaniko-context.json`
...
7. `modules/result.md` — write `.sealos/build/build-result.json`
...
| `write-result.mjs` | `node write-result.mjs --request <file> --status <status> --out <file> ...` | Write `.sealos/build/build-result.json` |
...
Validate it against the shared main deployment contract:

```text
../sealos-deploy/schemas/build-result.schema.json
```
