...
- When official application health checks are available, managed workloads must define `livenessProbe`, `readinessProbe`, and `startupProbe`, aligned with official endpoints/commands. Compose `healthcheck` conversion always emits `startupProbe` (from `start_period` when set; otherwise a 120s default window).
