...
function sampleDeliveryManifest(artifacts) {
  return {
    version: '1.0',
    generated_at: '2026-06-17T00:00:00.000Z',
    artifacts,
...
test('requires the catalog reference summary in the delivery manifest', () => {
  const result = validateArtifactData('delivery-manifest', sampleDeliveryManifest([
  ...
  assert.deepEqual(result.errors, [{
    path: '$.artifacts',
    message: 'must include .sealos/template-references.json',
  }])
})

test('accepts a delivery manifest with the catalog reference summary', () => {
  const result = validateArtifactData('delivery-manifest', sampleDeliveryManifest([
    '.sealos/analysis.json',
