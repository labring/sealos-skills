...
| `deploy-template.mjs` | `node deploy-template.mjs <template-path> [--dry-run] [--region <url>] [--kubeconfig <path>] [--args-json '{"KEY":"value"}'\|--args-file <file>] [--labels-json '{"k":"v"}']` | Resolve region from local auth or the active kubeconfig API server and post Template YAML without exposing argument values; `--labels-json` (or `SEALAI_DEPLOY_LABELS_JSON`) sends extra labels as the API `extraLabels` field so created resources carry task markers |
