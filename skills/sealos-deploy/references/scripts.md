...
| `match-official-template.mjs` | `node match-official-template.mjs --github-url <url> [--repo-name <name>]` | Phase 1 deterministic official-template matcher: compares catalog `gitRepo` declarations against the project `github_url`. Prints `{ official_template, reason, checked }`. Catalog failure degrades to `null` (never STOP). |
| `build-push.mjs` | `node build-push.mjs <work-dir> <repo> [--mode build\|push\|all] [--registry ghcr\|dockerhub] [--user <user>] [--context <dir>] [--dockerfile <path>] [--image <ref>] [--local-tag <ref>]` | Local Buildx helper. Phase 3 local path: `--mode build` then `--mode push`. Default `--mode all` builds and pushes (UPDATE). Sandbox uses `k8s-kaniko-job` instead. Docker Hub path assumes a public image at deploy time (warn the user: publicly pullable). `SEALOS_BUILD_TIMEOUT_MS` overrides the 30-minute build/push timeout. |
...
| `resolve-target-identity.mjs` | `node resolve-target-identity.mjs` | Phase 5/6 target identity: context, namespace, service account, app cloud domain (namespace Ingress hosts → config `app_domains` → console fallback with warning), and cert secret name. Prints JSON with `sources` and `warnings`. |
| `../docker-to-sealos/scripts/resolve-images.ts` | `node --experimental-strip-types resolve-images.ts --compose <file> [--extra ref1,ref2] [--output <file>] [--include-db]` | Phase 4 image resolution: digest-pins every non-database compose image for linux/amd64 (registry HTTPS, docker fallback; no crane) and captures image config (user, exposed ports) for securityContext emission. |
...
| `deploy-template.mjs` | `node deploy-template.mjs <template-path> [--dry-run] [--args-json '{"KEY":"value"}'\|--args-file <file>]` | Resolve the current region, enforce private sensitive-args files on POSIX, post a local template YAML, and emit an allowlisted result with credential values redacted. Phase 6 real create must not use `--dry-run` (Phase 5 owns server-side dry-run). Requests time out after 120s (`SEALOS_DEPLOY_TIMEOUT_MS` overrides); a timeout reports `unknown_create: true` for the Phase 6 unknown-create branch. |
...
| `sealos-live-smoke.mjs` | `node sealos-live-smoke.mjs --url <url> [--captcha-path <path>] [--login-method json-token\|cookie-json] [--login-path <path>] [--credentials-file <path>] [--username <user>] [--password <pass>] [--token-path <path>] [--auth-path <path>] [--missing-api-path <path>] [--missing-page-path <path>]` | Read-only or credentialed HTTP smoke test for the App URL, authenticated routes, and API/SPA negative probes. Prefer `--credentials-file` (mode-0600 JSON `{"username","password"}`) — argv values are visible to every local process. |
...
| `server-dry-run.ts` | `node --experimental-strip-types server-dry-run.ts --template <path> --context <ctx> --namespace <ns> --service-account <sa> --cloud-domain <domain> --cert-secret-name <secret> [--kubeconfig <path>] [--repair-authorization <path>] [--private-log <path>]` | Phase 5 target-cluster server-side dry-run. Defaults to `KUBECONFIG=~/.sealos/kubeconfig` and `--insecure-skip-tls-verify`. Prints JSON (`status`, `template_sha256`, warnings/failures). Requires Node 22+ and `npm install` in `skills/sealos-deploy` (`yaml`). |
