...
```bash
docker buildx build --platform linux/amd64 --load -t test-build:latest . 2>&1
```
...
**Build Phase**:
- [ ] Image builds successfully (`docker buildx build` exits 0)
...
```json
{
  ...
  "complexity_level": "L3",
  "build": {
    "command": "docker buildx build --platform linux/amd64 --load -t test-build:latest .",
