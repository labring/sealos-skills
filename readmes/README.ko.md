...
Sealos Skills는 Sealos Cloud 개발 및 배포를 중심으로 하는 플러그인 우선 스킬 팩입니다. AI 에이전트가 프로젝트를 검사하고 누락된 배포 산출물을 준비하도록 지원합니다. 개발을 위해 Sealos Cloud 데이터베이스와 객체 스토리지를 연결합니다. 컨테이너 이미지를 빌드하거나 재사용하고, 앱을 Sealos Cloud에 배포하고, 로컬 읽기 전용 캔버스에서 배포된 리소스를 보여줍니다.

`npx plugins add` 명령 하나로 감지된 모든 에이전트 도구에 플러그인이 설치됩니다. Codex와 Claude Code의 네이티브 marketplace 설치는 동일한 루트 `skills/**` 소스를 사용합니다. `skills.sh`와 Gemini CLI, Qwen Code 같은 컨텍스트 전용 확장 호스트도 이를 사용합니다.
...
명령 하나로 Sealos 플러그인이 감지된 모든 에이전트 도구(Claude Code, Cursor, Codex, Grok Build, Kimi Code, GitHub Copilot CLI, VS Code)에 설치됩니다. `npx plugins targets`를 실행하면 감지된 도구 목록을 볼 수 있고, `--target <tool>`을 전달하면 하나의 도구에만 설치할 수 있습니다.
...
플러그인은 루트 `skills/**` 디렉터리에서 8개 스킬 전부를 설치합니다. 각 스킬의 역할은 아래 **포함된 스킬**을 참고하세요.

Codex에 설치한 후 Codex에서 플러그인을 사용합니다.

- **Codex CLI:** `$sealos` 입력
- **Codex App:** 채팅 입력창 왼쪽 아래의 **+** 버튼을 클릭하고 **Plugins** → **Sealos** 선택
...
| 도구 | 설치 | 사용법 |
| --- | --- | --- |
| Codex CLI / Codex App | `npx plugins add https://github.com/labring/sealos-skills --target codex`, 또는 위의 **Codex에 네이티브 설치** 참고 | Codex CLI에서 `$sealos`, Codex App에서 **+** → **Plugins** → **Sealos** |
| Claude Code | `npx plugins add https://github.com/labring/sealos-skills --target claude-code`, 또는 위의 **Claude Code에 네이티브 설치** 참고 | `/sealos` |
| Cursor / Grok Build / Kimi Code / Copilot CLI / VS Code | `npx plugins add https://github.com/labring/sealos-skills` | 설치 후 호스트 명령 진입점 |
...
| Amp / 일반 저장소 가져오기 도구 | `https://github.com/labring/sealos-skills.git` 가져오기 | 호스트에 따라 달라집니다 |
...
프로젝트가 배포되어 있다면 설치된 플러그인 진입점을 통해 `sealos-canvas` 스킬을 사용합니다.

`/sealos-deploy`, `/sealos-database`, `/sealos-s3`는 직접 사용하는 `skills.sh` 스킬 진입점입니다. 플러그인 사용 시에는 Codex의 `$sealos` 또는 Claude Code의 `/sealos`를 사용해야 합니다.
...
- Sealos 템플릿 생성
- 배포 후 롤아웃 검증
- 앱을 사용할 수 있다고 보고하기 전에 실제 Sealos App URL, 로그, 웹 앱의 로그인/설정 흐름, 리소스 범위 검증

기존 배포가 존재하면 이후 실행은 인플레이스 업데이트 흐름으로 전환됩니다.
...
프로젝트가 배포되지 않았다면 Sealos Canvas가 중지되고 먼저 프로젝트를 배포하도록 안내합니다.

## 설정 작동 방식

플러그인 호환 또는 `skills.sh` 호환 AI 에이전트와 배포할 프로젝트만 있으면 됩니다.

배포, 데이터베이스 및 객체 스토리지 흐름에서 Sealos Skills는 다음을 수행합니다.

- Docker 및 `kubectl` 같은 도구의 사용 가능 여부 확인
- 필요할 때 사용자에게 Sealos 로그인 안내
- `sealos-cli`를 사용하여 Sealos Cloud 데이터베이스 생성, 연결 세부 정보 확인 및 데이터베이스 작업 수행
...
실제 배포에는 Sealos Cloud 계정과 컨테이너 레지스트리 접근 권한이 필요합니다. 스킬이 시작된 후에도 이 설정을 안내받을 수 있습니다. 데이터베이스 및 객체 스토리지 작업에는 Sealos Cloud 계정과 요청한 리소스를 생성할 수 있는 워크스페이스가 필요합니다.
...
Codex, Claude Code 및 지원되는 기타 에이전트 도구에서는 다음 이유로 플러그인 설치를 권장합니다.

- 모든 Sealos 스킬을 하나의 관리형 패키지로 설치
- 지원되는 에이전트 도구 전반에 동일한 스킬 제공
- 플러그인 메타데이터, 로고, 프롬프트, 명령 및 기능을 함께 관리
- 별도의 스킬 패키지 복사본을 유지할 필요 제거

## 이 저장소 유지 관리

아래 섹션은 저장소 관리자와 플러그인 게시자를 위한 내용입니다. 사용자에게는 필요하지 않습니다.
...
- `.qoder-plugin/plugin.json`은 Qoder 플러그인 인터페이스를 정의하고 8개의 Codex 스킬 전부를 명시적으로 노출합니다.
- `qoder.md`는 스킬 구현을 복사하지 않고 Qoder 수준의 라우팅 및 안전 지침을 제공합니다.
...
- `scripts/package-qoder-plugin.py`는 플러그인 매니페스트가 아카이브 루트에 있는 Qoder 호환 ZIP을 빌드합니다.
...
- [`.qoder-plugin/plugin.json`](../.qoder-plugin/plugin.json) — Qoder 플러그인 매니페스트
- [`qoder.md`](../qoder.md) — Qoder 플러그인 라우팅 및 안전 지침
...
- [`scripts/package-qoder-plugin.py`](../scripts/package-qoder-plugin.py) — Qoder ZIP 패키징 도구
