...
Подготовьте YAML развёртывания [Sealos Cloud](https://sealos.io) с помощью ИИ-агента.

Sealos Skills — это набор навыков с приоритетом плагина, предназначенный для разработки в Sealos Cloud. Он помогает ИИ-агенту исследовать проект, подготавливать артефакты развёртывания и YAML, подключать базы данных и объектное хранилище Sealos Cloud для разработки, создавать или повторно использовать образ контейнера и просматривать развёрнутые ресурсы на локальном холсте только для чтения. `sealos-deploy` подготавливает только YAML. Он останавливается после фазы 4 и не развёртывает и не обновляет рабочие нагрузки.
...
Вам нужен только ИИ-агент, совместимый с плагинами или skills.sh, и проект для подготовки YAML.

При подготовке YAML и работе с базами данных и объектным хранилищем Sealos Skills делает следующее:

- проверяет инструменты, такие как Docker и kubectl
- при необходимости помогает пользователю войти в Sealos
- использует sealos-cli для баз данных Sealos Cloud
- использует sealos-cli s3 для объектного хранилища Sealos
- отправляет необходимые образы linux/amd64 в GHCR и использует их digest в YAML

Для локальной подготовки YAML нужна учётная запись Sealos Cloud. GitHub CLI требует входа только при создании и отправке нового образа. Позднее утверждённый процесс развёртывания применяет YAML.

## Что выполняет Sealos Deploy

Для запроса подготовки YAML агент делает следующее:

- выполняет только фазы 0–4
- материализует исходный код и готовит контракты фаз
- ищет уникальное точное официальное совпадение в текущем каталоге kb-0.9
- материализует или генерирует YAML и запускает шлюз развёртывания в фазе 4
- записывает .sealos/template/index.yaml и останавливается

Sealos Deploy не собирает входные данные развёртывания. Он не запускает dry-run кластера. Он не создаёт ресурсы, не обновляет рабочие нагрузки и не проверяет поведение во время выполнения.
...
Для репозитория, уже развёрнутого в Sealos, агент делает следующее:

1. Читает .sealos/state.json, чтобы найти развёрнутое приложение.
2. Запрашивает пространство имён Sealos командами kubectl get только для чтения.
3. Запускает временный интерфейс холста на 127.0.0.1.
4. Выводит и открывает адрес локального интерфейса для проверки.

Если состояния развёртывания нет, Sealos Canvas останавливается. Используйте утверждённый процесс развёртывания для применения YAML. sealos-deploy подготавливает только YAML.

## Включённые навыки

Плагин и пакет `skills.sh` предоставляют один и тот же источник навыков:

- `sealos-deploy` — подготавливает YAML развёртывания Sealos из локального или GitHub проекта
