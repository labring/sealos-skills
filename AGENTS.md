...
- Obtain explicit user confirmation before Kubernetes, database, or bucket deletion; public-access changes; or credential rotation. A `sealos-deploy` invocation authorizes installation of dependencies required by its selected path; other system-tool installation still requires explicit confirmation.
